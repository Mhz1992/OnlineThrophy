...
>(({ className, ...props }, ref) => (
    <DrawerPrimitive.Overlay
        ref={ref}
        className={cn('fixed inset-0 z-[500] bg-black/20 backdrop-blur-sm', className)}
        {...props}
    />
));
...
>(({ className, children, ...props }, ref) => (
    <DrawerPortal>
        ...
        <DrawerPrimitive.Content
            ref={ref}
            className={cn(
                'fixed inset-x-0 bottom-0 z-[500] mt-24 flex h-auto flex-col rounded-t-[30px] border bg-background [&>div:first-child]:hidden',
                className,
            )}
            {...props}
        >
